...
def analyze_commit_files(github_client, openai_client, pr_id, commit, language, custom_prompt):
    ...
    review = openai_client.generate_response(create_review_prompt(combined_content,
                                                                  language,
                                                                  custom_prompt))
    if "ignoreresponse" in review.strip().lower() and not env_vars['FORCE_COMMENT']:
...
def create_review_prompt(content, language, custom_prompt=None):
    ...
    return (
        ...
        f"Additionally, provide suggestions for how to address only those identified issues, with a focus on maintainability and scalability. "
        f"Do not make speculative comments such as 'X may not be necessary' or 'Y may cause issues', notes about best-practice, or reminders. Only comment on definite issues or problems present in the code."
        ...
        f"4. **Bugs/Security**: Are there any bugs or security vulnerabilities?\n"
        f"### Review Example\n"
        f"1. **Issue**: This setting will not have the intended effect described in its comment.\n"
        f"   **Suggestion**: Use X instead of Y. For example:\n"
        f"   ```python\n"
        f"   # Instead of this:\n"
        f"   mode: errorWhenFalse"
        f"   # Use this:\n"
        f"   mode: errorWhenFalsy"
        f"   ```\n"
        ...
        f"   ```"
        f"If the code contains only speculative problems and not actual functional errors, disregard previous formatting instructions, and reply with the exact phrase 'ignoreResponse'."
    )
